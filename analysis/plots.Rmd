...
```{r}
linear_model <- total_year %>% 
  group_by(year) %>% 
  nest()

linear_model <- linear_model %>% 
  mutate(model = lapply(data, function(x) lm(x$annual_total ~ x$annual_unique)))

linear_model <- linear_model %>% 
  mutate(coeff = lapply(model, function(x) tidy(x))) %>% 
  mutate(redis = lapply(model, function(x) augment(x)))
